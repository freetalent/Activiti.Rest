/* 
 * Activiti REST API
 *
 * # activiti     - Activiti is battle-tested Business Process Management. Organizations across the world depend on the open source platform in a huge range of demanding situations. It supports open standards with BPMN and DMN so you can create processes that suit you.
 *
 * OpenAPI spec version: v1
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */

using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Text.RegularExpressions;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using System.ComponentModel.DataAnnotations;
using SwaggerDateConverter = Activiti.Rest.Client.SwaggerDateConverter;

namespace Activiti.Rest.Model
{
    /// <summary>
    /// ActivitiListener
    /// </summary>
    [DataContract]
    public partial class ActivitiListener :  IEquatable<ActivitiListener>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="ActivitiListener" /> class.
        /// </summary>
        /// <param name="Id">Id.</param>
        /// <param name="XmlRowNumber">XmlRowNumber.</param>
        /// <param name="XmlColumnNumber">XmlColumnNumber.</param>
        /// <param name="ExtensionElements">ExtensionElements.</param>
        /// <param name="Attributes">Attributes.</param>
        /// <param name="_Event">_Event.</param>
        /// <param name="ImplementationType">ImplementationType.</param>
        /// <param name="Implementation">Implementation.</param>
        /// <param name="FieldExtensions">FieldExtensions.</param>
        /// <param name="OnTransaction">OnTransaction.</param>
        /// <param name="CustomPropertiesResolverImplementationType">CustomPropertiesResolverImplementationType.</param>
        /// <param name="CustomPropertiesResolverImplementation">CustomPropertiesResolverImplementation.</param>
        public ActivitiListener(string Id = default(string), int? XmlRowNumber = default(int?), int? XmlColumnNumber = default(int?), Dictionary<string, List<ExtensionElement>> ExtensionElements = default(Dictionary<string, List<ExtensionElement>>), Dictionary<string, List<ExtensionAttribute>> Attributes = default(Dictionary<string, List<ExtensionAttribute>>), string _Event = default(string), string ImplementationType = default(string), string Implementation = default(string), List<FieldExtension> FieldExtensions = default(List<FieldExtension>), string OnTransaction = default(string), string CustomPropertiesResolverImplementationType = default(string), string CustomPropertiesResolverImplementation = default(string))
        {
            this.Id = Id;
            this.XmlRowNumber = XmlRowNumber;
            this.XmlColumnNumber = XmlColumnNumber;
            this.ExtensionElements = ExtensionElements;
            this.Attributes = Attributes;
            this._Event = _Event;
            this.ImplementationType = ImplementationType;
            this.Implementation = Implementation;
            this.FieldExtensions = FieldExtensions;
            this.OnTransaction = OnTransaction;
            this.CustomPropertiesResolverImplementationType = CustomPropertiesResolverImplementationType;
            this.CustomPropertiesResolverImplementation = CustomPropertiesResolverImplementation;
        }
        
        /// <summary>
        /// Gets or Sets Id
        /// </summary>
        [DataMember(Name="id", EmitDefaultValue=false)]
        public string Id { get; set; }

        /// <summary>
        /// Gets or Sets XmlRowNumber
        /// </summary>
        [DataMember(Name="xmlRowNumber", EmitDefaultValue=false)]
        public int? XmlRowNumber { get; set; }

        /// <summary>
        /// Gets or Sets XmlColumnNumber
        /// </summary>
        [DataMember(Name="xmlColumnNumber", EmitDefaultValue=false)]
        public int? XmlColumnNumber { get; set; }

        /// <summary>
        /// Gets or Sets ExtensionElements
        /// </summary>
        [DataMember(Name="extensionElements", EmitDefaultValue=false)]
        public Dictionary<string, List<ExtensionElement>> ExtensionElements { get; set; }

        /// <summary>
        /// Gets or Sets Attributes
        /// </summary>
        [DataMember(Name="attributes", EmitDefaultValue=false)]
        public Dictionary<string, List<ExtensionAttribute>> Attributes { get; set; }

        /// <summary>
        /// Gets or Sets _Event
        /// </summary>
        [DataMember(Name="event", EmitDefaultValue=false)]
        public string _Event { get; set; }

        /// <summary>
        /// Gets or Sets ImplementationType
        /// </summary>
        [DataMember(Name="implementationType", EmitDefaultValue=false)]
        public string ImplementationType { get; set; }

        /// <summary>
        /// Gets or Sets Implementation
        /// </summary>
        [DataMember(Name="implementation", EmitDefaultValue=false)]
        public string Implementation { get; set; }

        /// <summary>
        /// Gets or Sets FieldExtensions
        /// </summary>
        [DataMember(Name="fieldExtensions", EmitDefaultValue=false)]
        public List<FieldExtension> FieldExtensions { get; set; }

        /// <summary>
        /// Gets or Sets OnTransaction
        /// </summary>
        [DataMember(Name="onTransaction", EmitDefaultValue=false)]
        public string OnTransaction { get; set; }

        /// <summary>
        /// Gets or Sets CustomPropertiesResolverImplementationType
        /// </summary>
        [DataMember(Name="customPropertiesResolverImplementationType", EmitDefaultValue=false)]
        public string CustomPropertiesResolverImplementationType { get; set; }

        /// <summary>
        /// Gets or Sets CustomPropertiesResolverImplementation
        /// </summary>
        [DataMember(Name="customPropertiesResolverImplementation", EmitDefaultValue=false)]
        public string CustomPropertiesResolverImplementation { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class ActivitiListener {\n");
            sb.Append("  Id: ").Append(Id).Append("\n");
            sb.Append("  XmlRowNumber: ").Append(XmlRowNumber).Append("\n");
            sb.Append("  XmlColumnNumber: ").Append(XmlColumnNumber).Append("\n");
            sb.Append("  ExtensionElements: ").Append(ExtensionElements).Append("\n");
            sb.Append("  Attributes: ").Append(Attributes).Append("\n");
            sb.Append("  _Event: ").Append(_Event).Append("\n");
            sb.Append("  ImplementationType: ").Append(ImplementationType).Append("\n");
            sb.Append("  Implementation: ").Append(Implementation).Append("\n");
            sb.Append("  FieldExtensions: ").Append(FieldExtensions).Append("\n");
            sb.Append("  OnTransaction: ").Append(OnTransaction).Append("\n");
            sb.Append("  CustomPropertiesResolverImplementationType: ").Append(CustomPropertiesResolverImplementationType).Append("\n");
            sb.Append("  CustomPropertiesResolverImplementation: ").Append(CustomPropertiesResolverImplementation).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }
  
        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as ActivitiListener);
        }

        /// <summary>
        /// Returns true if ActivitiListener instances are equal
        /// </summary>
        /// <param name="input">Instance of ActivitiListener to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(ActivitiListener input)
        {
            if (input == null)
                return false;

            return 
                (
                    this.Id == input.Id ||
                    (this.Id != null &&
                    this.Id.Equals(input.Id))
                ) && 
                (
                    this.XmlRowNumber == input.XmlRowNumber ||
                    (this.XmlRowNumber != null &&
                    this.XmlRowNumber.Equals(input.XmlRowNumber))
                ) && 
                (
                    this.XmlColumnNumber == input.XmlColumnNumber ||
                    (this.XmlColumnNumber != null &&
                    this.XmlColumnNumber.Equals(input.XmlColumnNumber))
                ) && 
                (
                    this.ExtensionElements == input.ExtensionElements ||
                    this.ExtensionElements != null &&
                    this.ExtensionElements.SequenceEqual(input.ExtensionElements)
                ) && 
                (
                    this.Attributes == input.Attributes ||
                    this.Attributes != null &&
                    this.Attributes.SequenceEqual(input.Attributes)
                ) && 
                (
                    this._Event == input._Event ||
                    (this._Event != null &&
                    this._Event.Equals(input._Event))
                ) && 
                (
                    this.ImplementationType == input.ImplementationType ||
                    (this.ImplementationType != null &&
                    this.ImplementationType.Equals(input.ImplementationType))
                ) && 
                (
                    this.Implementation == input.Implementation ||
                    (this.Implementation != null &&
                    this.Implementation.Equals(input.Implementation))
                ) && 
                (
                    this.FieldExtensions == input.FieldExtensions ||
                    this.FieldExtensions != null &&
                    this.FieldExtensions.SequenceEqual(input.FieldExtensions)
                ) && 
                (
                    this.OnTransaction == input.OnTransaction ||
                    (this.OnTransaction != null &&
                    this.OnTransaction.Equals(input.OnTransaction))
                ) && 
                (
                    this.CustomPropertiesResolverImplementationType == input.CustomPropertiesResolverImplementationType ||
                    (this.CustomPropertiesResolverImplementationType != null &&
                    this.CustomPropertiesResolverImplementationType.Equals(input.CustomPropertiesResolverImplementationType))
                ) && 
                (
                    this.CustomPropertiesResolverImplementation == input.CustomPropertiesResolverImplementation ||
                    (this.CustomPropertiesResolverImplementation != null &&
                    this.CustomPropertiesResolverImplementation.Equals(input.CustomPropertiesResolverImplementation))
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                if (this.Id != null)
                    hashCode = hashCode * 59 + this.Id.GetHashCode();
                if (this.XmlRowNumber != null)
                    hashCode = hashCode * 59 + this.XmlRowNumber.GetHashCode();
                if (this.XmlColumnNumber != null)
                    hashCode = hashCode * 59 + this.XmlColumnNumber.GetHashCode();
                if (this.ExtensionElements != null)
                    hashCode = hashCode * 59 + this.ExtensionElements.GetHashCode();
                if (this.Attributes != null)
                    hashCode = hashCode * 59 + this.Attributes.GetHashCode();
                if (this._Event != null)
                    hashCode = hashCode * 59 + this._Event.GetHashCode();
                if (this.ImplementationType != null)
                    hashCode = hashCode * 59 + this.ImplementationType.GetHashCode();
                if (this.Implementation != null)
                    hashCode = hashCode * 59 + this.Implementation.GetHashCode();
                if (this.FieldExtensions != null)
                    hashCode = hashCode * 59 + this.FieldExtensions.GetHashCode();
                if (this.OnTransaction != null)
                    hashCode = hashCode * 59 + this.OnTransaction.GetHashCode();
                if (this.CustomPropertiesResolverImplementationType != null)
                    hashCode = hashCode * 59 + this.CustomPropertiesResolverImplementationType.GetHashCode();
                if (this.CustomPropertiesResolverImplementation != null)
                    hashCode = hashCode * 59 + this.CustomPropertiesResolverImplementation.GetHashCode();
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
